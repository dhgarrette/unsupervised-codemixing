...
//      "the man walks the dog",
    ...
    val trInit =
      new SimpleConditionalLogProbabilityDistribution[Symbol, Symbol](
        Map(
          ...
          'D -> new SimpleLogProbabilityDistribution(Map('N -> 0.7, 'V -> 0.1, tagdict.endTag -> 0.1).mapVals(LogDouble(_))),
          'N -> new SimpleLogProbabilityDistribution(Map('D -> 0.1, 'V -> 0.4, tagdict.endTag -> 0.2).mapVals(LogDouble(_))),
          'V -> new SimpleLogProbabilityDistribution(Map('D -> 0.3, 'N -> 0.2, tagdict.endTag -> 0.4).mapVals(LogDouble(_))),
    ...
    val emt =
      new EdgeRestrictedSemisupervisedTaggerTrainer(
        ((pt: Symbol) => ((ct: Symbol) => pt != ct)),
        new SoftEmHmmTaggerTrainer[Symbol](2, new UnsmoothedTransitionDistributioner[Symbol](), new UnsmoothedEmissionDistributioner[Symbol](), alphaT = 0.0, alphaE = 0.0, 1e-10))
    ...
    val em = hmm.emissions
        ...
        for (t1 <- (tagdict.allTags ++ Set('A, 'Z)).toVector.sortBy(_.toString))
          println(f"$t1 -> ${(tagdict.allTags ++ Set('A, 'Z)).toVector.sortBy(_.toString).mapTo(t2 => f"${tr(t2, t1).toDouble}%.10f").mkString(" ")}")
        for (t <- (tagdict.allTags ++ Set('A, 'Z)).toVector.sortBy(_.toString))
          println(f"$t -> ${tagdict.allWords.toVector.sortBy(_.toString).mapTo(w => f"${em(w, t).toDouble}%.10f").mkString(" ")}")
